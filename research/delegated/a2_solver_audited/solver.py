...
class ReducedRejected(Exception):
    """A trial lacks a physical approximation certificate; never accept it."""
...
class RunState:
    ...
    stage_evaluations: list[int] = field(default_factory=lambda: [0,0,0,0])
...
def make_exact_fg(
    ...
    ratio = run.ledger.calibration.matvec_ratio() if run.ledger.calibration else 0.0
    f_est = (12.0 + 60.0 * ratio) * len(freq_ids)
    ...
    def fg(z: np.ndarray):
        ...
        run.stage_evaluations[guard.stage_idx] += 1
...
def make_exact_f_only(
    ...
    def f(z: np.ndarray):
        z = np.asarray(z, dtype=float)
        ratio = run.ledger.calibration.matvec_ratio() if run.ledger.calibration else 0.0
        guard.check((6.0 + 18.0 * ratio) * len(freq_ids), "exact objective-only evaluation")
        alpha, x = _params_from_scaled(z, lam)
        pt = prior_terms(alpha, run.settings)
        if phaseless:
            out = phaseless_loss(
...
def _reduced_trial_units(
    ...
    # All counted dense and skinny products are conservatively charged as
    # dense matvec equivalents; SVD/QR setup remains visible in wall time.
    return rr * 78.0 * F + qr * F + (rank + 6.0 * (3 + 2*12 + 1 + 12 + 3))*F*cal.matvec_ratio()
...
def run_reduced_stage(
    ...
    while True:
        ...
        if not fixed and run.ledger.units + est > guard.cap:
            run.mode = "exact"
            run.fallback_events.append({"stage":stage_idx,"reason":"reduced_probe_unaffordable_direct_fallback"})
            continue
        ...
        if cert is not None and not cert["pass"]:
            if int(settings.get("max_reduced_probes", 1)) <= 1:
                run.mode = "exact"
                run.fallback_events.append({"stage": stage_idx,
                    "reason": "certificate_refused_one_probe_then_direct", "rank": chart_rank})
                continue
        ...
        bounds = _bounds(lam)
        tracker = Tracker(z0)
        # Reserve an exact checkpoint at both endpoints. Approximate f/g
        # values alone must never release an uncertified physical iterate.
        if not fixed:
            exact_before = make_exact_f_only(model, y, sigma, freq_ids, run, guard, False)(z0)
        ...
        def fg(z: np.ndarray):
            ...
            if not fixed:
                from cert import reduced_certification
                ct = reduced_certification(model, alpha, y_sel, sigma, freq_ids, out, settings)
                if not ct["pass"]:
                    raise ReducedRejected("trial residual/gradient certificate refused")
        ...
        except ReducedRejected:
            run.alpha, run.x = _params_from_scaled(z0, lam)
            run.mode = "exact"
            run.fallback_events.append({"stage": stage_idx,"reason": "uncertified_trial_direct_fallback"})
            continue
        except StageLimit:
            run.alpha, run.x = _params_from_scaled(tracker.current if fixed else z0, lam)
            run.incomplete_stages.append(stage_idx)
            return
        except BudgetExhausted:
            run.alpha, run.x = _params_from_scaled(tracker.current if fixed else z0, lam)
        ...
        if not fixed:
            exact_after = make_exact_f_only(model, y, sigma, freq_ids, run, guard, False)(z_new)
            if exact_after >= exact_before:
                run.mode = "exact"
                run.fallback_events.append({"stage":stage_idx,"reason":"nondecreasing_exact_trial_rejected"})
                continue
        run.alpha = alpha_new
        run.x = x_new
        run.reduced_moves += 1
        run.stage_evaluations[stage_idx] += 1
...
def solve(
    ...
    try:
        ...
        # This guard stops BEFORE additional work is done and returns the
        # last accepted state. Budget stopping is not optimizer failure;
        # missing the final data stage still fails the completion gate.
        run.budget_stop = True
        if run.stage_evaluations[-1] == 0:
            run.failure = f"final_stage_not_evaluated: {exc}"
    ...
    return {
        ...
        "wall_actual": time.perf_counter()-t_start,
        "stage_evaluations": run.stage_evaluations,
        "budget_stop": run.budget_stop,
        "finished_budget_policy": run.stage_evaluations[-1] > 0 and run.failure is None,
    }
