...
def prepare_scene_impl(args, models, seed, inits):
    ...
    m16._calibration = Calibration.from_dict(frozen["calibration"]) if frozen is not None else Calibration_from_path(SOLVER_DIR / "calibration_tune.json")
...
def run_one(
    ...
    success = bool(
        failure is None
        and res.get("finished_budget_policy", False)
    ...
    rec = {
        ...
        "failure": failure,
        "status": status,
        "finished_budget_policy": res.get("finished_budget_policy",False),
        "stage_evaluations":res.get("stage_evaluations",[]),
        "success": success,
        "mode": res.get("mode"),
        "rank": res.get("rank"),
...
def summarize(records_path: Path, out_dir: Path, methods: list[str] | None):
    ...
    if any(r.get("seed", 0) in FINAL_SEED_RANGE for r in rows):
        ...
        lines += ["", "## Final-seed paired statistics"] + final_stats.get("tables", [])
