...
def lowpass_task_operator(model32, ell_res: float) -> np.ndarray:
    ...
    h = model32.h
    Phi = model32.basis
    # radial Gaussian smoothing kernel with sigma=ell_res/4 is conservative:
    # its 4-sigma support is the declared resolution length.
    sigma = float(ell_res) / 4.0
...
def reference_metrics(
    ...
    Jx = JR[:, 9:]
    Ju = Jx * Tc[None, :]
    C_u = np.linalg.inv(Ju.T @ Ju) if full_rank else np.linalg.pinv(Ju.T @ Ju)
