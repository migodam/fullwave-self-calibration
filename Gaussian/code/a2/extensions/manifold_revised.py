...
OUT=ROOT/'runs/a2/extensions/manifold_v3';OUT.mkdir(parents=True,exist_ok=True)
FIG=ROOT/'figures/a2/extensions';FIG.mkdir(parents=True,exist_ok=True)
CFG=dict(seed=20260916,n_inverse=32,n_data=64,frequencies=[1.5e9,2.75e9],n_tx=6,n_rx=24,noise_relative=.01,families=['separated','close','sharp'],cases_per_family=10,max_outer=100,max_rhs=3600,trust_radius=.7,translation_scale_m=.03,parameter_weights=[1,1,1],min_relative_decrease=1e-9,methods=['full_lm','spectral','weak_secant','random_secant'],scope='2D scalar; two fixed Gaussian components; known loss ratio; fixed common ROI initialization; half receive/full transmit aperture; independent grid and quadrature')
def sqrtm(S):
...
def fit(solvers,y,mask,method,seed,truth_map=None,clean=None):
 ...
 for iteration in range(CFG['max_outer']):
  ...
  if model.rhs+144+(60 if method in ('weak_secant','random_secant') else 12)>CFG['max_rhs']:break
  ...
  if method in ('weak_secant','random_secant') and rank<len(s):
   if method=='weak_secant':direction=Vh[rank].copy()
   else:
    direction=rng.normal(size=len(s));direction-=V@(V.T@direction);direction/=np.linalg.norm(direction)
   assert np.linalg.norm(V.T@direction)<1e-8
   for sign in (-1,1):
    h=sign*.5*direction;sp=retract(state,h)
    if sp is None:continue
    fp,_=model.field(sp);probe_count+=1;delta=real((fp-f)[mask])/scale
    correction=visible(r-delta);limit=np.sqrt(CFG['trust_radius']**2-np.linalg.norm(h)**2);correction*=min(1,limit/max(np.linalg.norm(correction),1e-30));proposals.append(h+correction)
  ...
  for step in proposals:
   ...
   if val<best[0]-CFG['min_relative_decrease']*max(best[0],1e-12):best=(val,sp,fp,fwp,rp);accepted=True
  hist.append(dict(iteration=iteration,loss=best[0],rank=rank,rhs=model.rhs,accepted=accepted,probe_fields=probe_count,no_weak_complement=(rank==len(s)),material_relative_l2=None if truth_map is None else float(np.linalg.norm(material(solvers[0].points,best[1])-truth_map)/np.linalg.norm(truth_map)),heldout_clean_relative=None if clean is None else float(np.linalg.norm((best[2]-clean)[~mask])/np.linalg.norm(clean[~mask]))))
 ...
 for family in CFG['families']:
  for rep in range(10):
   ...
   for method in CFG['methods']:
    st,f,meta=fit(inv,y,mask,method,seed,itruth,clean);c=material(inv[0].points,st);meta.update(material_relative_l2=float(np.linalg.norm(c-itruth)/np.linalg.norm(itruth)),heldout_clean_relative=float(np.linalg.norm((f-clean)[~mask])/np.linalg.norm(clean[~mask])),train_clean_relative=float(np.linalg.norm((f-clean)[mask])/np.linalg.norm(clean[mask])))
...
def plot(records,images):
 ...
 fig.colorbar(obj,ax=axes.ravel().tolist(),label='real contrast',shrink=.7);fig.savefig(FIG/'manifold_v3_reconstructions.png',dpi=170,bbox_inches='tight');plt.close(fig)
