...
the disk of area h^2, radius a = h/sqrt(pi):
      ...
      g_self = (i/4) * 2 pi * (a/k_b) * H_1^{(1)}(k_b a)

The diagonal Green entry of G_D is k_b^2 * g_self  (NOT multiplied again by
h^2; g_self already carries the cell area factor).  Off-diagonal entries are
k_b^2 h^2 g(...).  This mirrors the stated rule
      G_D = k_b^2 * (Green_matrix_NxN * h^2),
      diagonal entries = k_b^2 * g_self.
...
q g(z_n, s_tx) with respect to the transmitter position s_tx is
      ...
      grad_2 g(z, s) = +(i k_b/4) H_1^{(1)}(k_b R) (z - s)/R
                    = - green_grad_first(s, z)  for E_inc.

Writing the second argument s = z - (z - s), d g(z, s)/ds =
-d g(z, s)/dz, and the numerical transmitter-position sweep confirms the plus
sign (a relative discrepancy of exactly 2 if the minus sign is used).  The
code below therefore applies the minus sign to the first-argument gradient.
...
def self_cell_green(k_b: float, h: float) -> complex:
    """Disk-integrated self-cell Green value g_self (area units, no h^2 factor).

    g_self = (i/4) * 2*pi * (a/k_b) * H_1^{(1)}(k_b a),  a = h/sqrt(pi).
    """
    k_b = float(k_b)
...
def green_grad_first(src: np.ndarray, dst: np.ndarray, k_b: float) -> np.ndarray:
    ...
    This same expression gives grad_2 g(z, s) when the first argument is the
    grid point z and the second argument the source s, because both are
    -(i k_b/4) H_1^{(1)}(k_b R) (first - second)/R.
    """
    src = np.atleast_2d(np.asarray(src, dtype=float))
    dst = np.atleast_2d(np.asarray(dst, dtype=float))
    if src.shape[1] != 2 or dst.shape[1] != 2:
...
def build_AB(
    ...
    for t in range(T):
        ...
        # grad_2 g(z_n, s_tx) = -(i k_b/4) H1 (s - z)/R
        #                     = - green_grad_first(s_tx, z_n)   (see SIGN NOTE)
        grad2 = -green_grad_first(tx_t[None, :], points, k_b)[:, 0, :]  # (S, 2)
