...
## Mandatory round-3 parent override (2026-09-04)

Before any further development, read `context/PARENT_ROUND3_AUDIT.md` in full.
It overrides conflicting family numbers and claim wording in the current idea,
protects existing Family 10/11 and parent-correction artifacts from overwrite,
and fixes the generalized-retention/rank-event/writeup gates for Round 3.

Also read `context/PARENT_FAMILY12_REVIEW.md` in full before interpreting or
writing up Family 12.  It fixes the trajectory cross-check transcription,
distinguishes finite-prior retention from principal angles, renames thresholded
log-volume as positive-support log-pseudovolume, and requires factored rather
than normal-matrix rank tests.

Before interpreting or writing up Family 15, read
`context/PARENT_FAMILY15_REVIEW.md` in full.  It requires `(A_born | B_full)`
to be labelled a mixed-tangent isolation control rather than a complete Born
SLAM model, preserves the missing `B_born` control, and distinguishes a Born
linear-model covariance from misspecification MSE and the exact-model CRB.
