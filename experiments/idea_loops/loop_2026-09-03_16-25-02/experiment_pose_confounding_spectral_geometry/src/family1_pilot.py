...
Self-cell formula marker (parent correction gate 2026-09-03, workshop rule
16): this run uses the complete equal-area disk integral
I_self = (i*pi*a/(2*k_b))*H_1^(1)(k_b*a) - 1/k_b^2, a = h/sqrt(pi), including
the -1/k_b^2 lower-endpoint term.  The formula and version string are written
into the result JSON as ``self_cell_formula`` / ``self_cell_formula_version``.
The earlier incomplete formula (which omits -1/k_b^2) was archived under
``archive/INVALIDATED_self_cell_2026-09-03T092339Z/``.
...
def main() -> None:
    ...
    results = {
        ...
        "command": ".venv/bin/python src/family1_pilot.py",
        ...
        "source_sha256": {
            "src/helmholtz.py": hashlib.sha256(
                (_HERE / "helmholtz.py").read_bytes()
            ).hexdigest(),
            "src/family1_pilot.py": hashlib.sha256(
                (_HERE / "family1_pilot.py").read_bytes()
            ).hexdigest(),
        },
        "config": {
            **cfg,
    ...
    ax.set_title(
        ...
        f"(N={cfg['N']}, k_b=2pi, T={cfg['T']}, n_rx={cfg['n_rx']}, "
        "two-blob chi0; corrected equal-area self-cell v2)"
    )
